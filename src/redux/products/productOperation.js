...
const addProduct = (date, productId, weight) => (dispatch) => {
  ...
  axiosInstance
    ...
    .then((response) =>
      dispatch(productAction.addProductSuccess(response.data))
    )
...
const fetchProduct = (data) => (dispatch, getState) => {
  ...
  axiosInstance
    .post("/day/info", data)
    .then((response) =>
      dispatch(productAction.fetchProductSuccess({ ...response.data, ...data }))
    )
    .catch((error) => {
...
const deleteProduct = (dayId, eatenProductId) => (dispatch, getState) => {
  ...
  axiosInstance
    ...
    .then((response) =>
      dispatch(
        productAction.deleteProductSuccess({ ...response.data, eatenProductId })
      )
    )
    .catch((error) => {
      dispatch(productAction.fetchProductError(error.message));
